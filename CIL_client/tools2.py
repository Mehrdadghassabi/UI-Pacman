...
def set_requirements(main_grid, grid_height, grid_width, turn, max_turn, score):
    ...
    if len(grid.path_to_go) == 0 and not grid.teleporting:
        ...
        grid.set_point()
        maximum = x_goal = y_goal = -1

        if not gems_visible() or not grid.gems:
            exit('No gem remains')
...
def check_collected(x, y):
    if grid.grid[x][y].value in ['1', '2', '3', '4'] and grid.grid[x][y].visibility:
        grid.gems_collected[ord(grid.primary_grid[x][y].value) - 49] += 1
    for i in range(len(grid.gems_collected)):
        if grid.gems_collected[i] == grid.gem_limits[i]:
            grid.minimums[i] = 1000
